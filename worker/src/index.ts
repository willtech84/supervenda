    ...
    try {
      ...
      if (tableMap[entity] && req.method === "POST") {
        ...
        if (entity === "produtos") {
          ...
          const row = await env.DB.prepare("SELECT * FROM produtos WHERE id=? AND vendor_id=?").bind(id, vendorId).first();
          return json(row);
        }
        ...
        if (entity === "despesas") {
          ...
          const row = await env.DB.prepare("SELECT * FROM despesas WHERE id=? AND vendor_id=?").bind(id, vendorId).first();
          return json(row);
        }
        ...
        if (entity === "lembretes") {
          ...
          const row = await env.DB.prepare("SELECT * FROM lembretes WHERE id=? AND vendor_id=?").bind(id, vendorId).first();
          return json(row);
        }
